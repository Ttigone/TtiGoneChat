...
bool Databases::openDb() {
  if (!database_.open()) {
    LOG_DEBUG() << "Error: Failed to connect database."
                << database_.lastError();
  } else {
    LOG_DEBUG() << "成功打开";
  }
...
bool Databases::createTable(const QString& query_sen,
  ...
  if (isTableExist(table_name)) {
    LOG_INFO() << table_name << " Table already exists.";
    return true;
  }
  //qDebug() << "test";

  QSqlQuery query(database_);
  query.prepare(query_sen);
  if (!query.exec()) {
...
void Databases::exec(const QString& sen) {
  if (!database_.isOpen()) {
    LOG_DEBUG() << "Error: Fail to open database." << database_.lastError();
    return;
  }
...
bool Databases::isTableExist(const QString& table_name) {
  ...
  if (!query.exec()) {
    LOG_DEBUG() << "Error: Failed to check if table exists"
                << query.lastError();
    return false;
  }
...
Databases::Databases(const DatabaseConfig& cfg) : config_(cfg) {
  // 是否已经存在默认数据库连接

...
template <typename T>
inline std::pair<QString, QString> generateCreateTableQuery(const QString &table_name) {
...
class Databases {
 public:
  ...
  void deleteDataById(const QString &table_name, int64 id) {
    if (!database_.isOpen()) {
      LOG_FATAL() << "Error: Fail to delete data. Because the databases is not opened"
                  << database_.lastError();
      return;
    }
    QString queryStr = QString("DELETE FROM %1 WHERE %2 = :id").arg(table_name).arg(primary_key_.toString());
    QSqlQuery query(database_);
    query.prepare(queryStr);
  ...
  template <typename T>
  std::vector<T> queryAllData(const QString &table_name)
  {
    std::vector<T> dataList;
    if (!database_.isOpen())
		{
			LOG_FATAL() << "Error: Fail to query all data. Because the databases is not opened"
									<< database_.lastError();
			return dataList;
		}

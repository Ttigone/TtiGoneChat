...
namespace QWK {
    ...
    static inline MONITORINFOEXW getMonitorForWindow(HWND hwnd) {
        ...
        HMONITOR monitor = ::MonitorFromWindow(hwnd, MONITOR_DEFAULTTONEAREST); // 获取窗口相邻最近的显示器句柄
        MONITORINFOEXW monitorInfo{};   // 显示器信息函数
        monitorInfo.cbSize = sizeof(monitorInfo);   // 设置大小 也可以是 结构体名
        ::GetMonitorInfoW(monitor, &monitorInfo);   // 获取显示器信息 存储到 monitorInfo 结构体中
        return monitorInfo;
    }

    // 窗口移动 到显示器  窗口在多个显示器中跨越
    static inline void moveWindowToMonitor(HWND hwnd, const MONITORINFOEXW &activeMonitor) {
        RECT currentMonitorRect = getMonitorForWindow(hwnd).rcMonitor;  // 窗口所在 显示器坐标区域
        RECT activeMonitorRect = activeMonitor.rcMonitor;   // 当前活动的显示器坐标区域
        // We are in the same monitor, nothing to adjust here.
        if (currentMonitorRect == activeMonitorRect) {  // 处于相同的显示器 无需移动
            return;
        }
        // 需要移动
        ...
        // ::SetWindowPos(hwnd, HWND_TOPMOST, newWindowX, newWindowY, RECT_WIDTH(currentWindowRect),
        //                RECT_HEIGHT(currentWindowRect),
        //                SWP_NOACTIVATE | SWP_NOSIZE | SWP_NOZORDER | SWP_NOOWNERZORDER);
    }
    ...
    static inline void bringWindowToFront(HWND hwnd) {
        // 获取前台窗口  用户上一时刻正在操作的窗口  但下一个时刻需要使用当 我们的窗口
        HWND oldForegroundWindow = ::GetForegroundWindow();     // 用户正在操作的窗口
        ...
        // 最小化窗口 需要恢复到之前的状态 原始大小和位置
        if (IsMinimized(hwnd)) {
            // Restore the window if it is minimized.
            // 激活并显示窗口。如果窗口被最小化、最大化或排列，系统会将其恢复到原来的大小和位置。应用程序在还原最小化窗口时应指定此标志。
       ...
       // 以下操作都是为了将 被叠放在下面的窗口 
        ...
        // 窗口没有最小化，但是 Z 轴顺序 高,也就是位于其他窗口之后, 需要将其升级为 前台窗口
        // 函数失败，返回 0 代表窗口挂起
        // 检测上一个前台窗口是否为挂起状态，挂起的话则不能活动我们的当前窗口，直接退出
        // oldForegroundWindow 处理了 WM_NULL 信息，则返回非零值
        ...
        // 获取线程 id   窗口由线程创建  oldForgroundWindow 前台窗口 的 线程 id
        ...
        // 假装前台窗口 是 我们创建
        // 将 前台窗口 线程 附加到 我们窗口 线程
        ::AttachThreadInput(windowThreadProcessId, currentThreadId, TRUE);
        
        // 可能不会被使用 离开当前作用域时 执行 线程分离的操作

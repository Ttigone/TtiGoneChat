...
namespace QWK {
    ...
    void WindowBarPrivate::init() {
        Q_Q(WindowBar); // 获取 q 指针，该 q 指针是原始类的 this 指针
        layout = new QHBoxLayout();
        if (QLocale::system().textDirection() == Qt::RightToLeft) { // 判断并设置语言的文本方向
            layout->setDirection(QBoxLayout::RightToLeft);
        }
        
        layout->setContentsMargins(QMargins()); // 设置 无矩边
        layout->setSpacing(0);  // 布局中的小布局间距 为 0    // 默认间距适应屏幕
        for (int i = IconButton; i <= CloseButton; ++i) {
            insertDefaultSpace(i);  // 插入 空白 0w, 0h
        }
        q->setLayout(layout);
    }
    ...
    void WindowBar::setHostWidget(QWidget *w) {
        Q_D(WindowBar); // 获取隐私类的指针 d

        QWidget *org = d->w;    // 访问 隐私成员
        if (org) {
            org->removeEventFilter(this);   // 移除隐私类指针的 事件过滤器
        }
        d_ptr->w = w;   // 直接的隐私类指针  存储父窗口 ?
        if (w) {
            w->installEventFilter(this);    // 安装事件过滤器
        }
    }

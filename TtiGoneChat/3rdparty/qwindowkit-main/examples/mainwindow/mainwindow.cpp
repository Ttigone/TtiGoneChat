...
void MainWindow::installWindowAgent() {
    ...
    auto menuBar = [this]() {
        auto menuBar = new QMenuBar();  // 创建菜单栏
        ...
        connect(darkAction, &QAction::triggered, this, [this](bool checked) {
            loadStyleSheet(checked ? Dark : Light); // 根据勾选状态确定 加载的样式表
        });
        connect(this, &MainWindow::themeChanged, darkAction, [this, darkAction]() {
            darkAction->setChecked(currentTheme == Dark); // 样式表改变，勾选状态同时改变  默认是 dark
        });
    ...
    auto windowBar = new QWK::WindowBar();  // 创建标题栏
#ifndef Q_OS_MAC
    // 为标题栏设置 对应的按钮
    ...
    windowAgent->setTitleBar(windowBar);    // 设置标题栏
#ifndef Q_OS_MAC
    // 为什么还要设置一遍
